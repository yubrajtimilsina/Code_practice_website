...
export default function ChallengeHistory() {
  ...
  const fetchHistory = async (page = 1) => {
    try {
      setLoading(true);
      const response = await getMyHistory(page, pagination.limit);
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {challenge.problemId?.tags?.slice(0, 2).map((tag, index) => (
                              <span key={`${tag}-${index}`} className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-xs">
                                {tag}
                              </span>
                            ))}
                          </div>
